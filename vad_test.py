...
"""During inference, we perform frame-level prediction by two approaches: 1) shift the window of length
window_length_in_sec (e.g. 0.63s) by shift_length_in_sec (e.g. 10ms) to generate the frame and use the prediction of
the window to represent the label for the frame; [this script demonstrate how to do this approach] 2) generate
predictions with overlapping input segments. Then a smoothing filter is applied to decide the label for a frame
spanned by multiple segments. [get frame level prediction by this script and use vad_overlap_posterior.py in
NeMo/scripts/voice_activity_detection One can also find posterior about converting frame level prediction to
speech/no-speech segment in start and end times format in that script.]
...
Usage: python vad_infer.py --config-path="../conf/vad" --config-name="vad_inference_postprocessing.yaml"
...
# @hydra_runner(config_path="/home/chita/workstation/NeMo/configs/", config_name="vad_inference_postprocessing.yaml")


def vad(cfg):

    global table_out_dir, root
    print(cfg)
    if not cfg['dataset']:
        raise ValueError("You must input the path of json file of evaluation data")

    # each line of dataset should have different audio_filepath and unique name to simplify edge cases or conditions
    key_meta_map = {}
    ...
    if not os.path.exists(cfg['frame_out_dir']):
        ...
        logging.warning(
            '''Note frame_out_dir exists. If new file has same name as file inside existing folder, it will append 
               result to existing file and might cause mistakes for next steps.'''
        )
    ...
    if cfg['gen_seg_table']:
        ...
        logging.info(
            f"Finish generating speech segments table with postprocessing_params: {cfg['vad']['parameters']['postprocessing']}"
        )
    ...
    if cfg['write_to_manifest']:
        ...
        if not cfg['out_manifest_filepath']:
            out_manifest_filepath = "vad_out29.json"
        else:
            root = os.path.dirname(os.path.realpath(__file__))
        ...
        return root + f'/{out_manifest_filepath}'
